...
export default function TeamDashboard() {
  ...
  const [leaderData, setLeaderData] = useState<TeamMemberWithProfile | null>(null);
  ...
  useEffect(() => {
    ...
    startTransition(() => {
      // Get team data
      apiClient.get(`teams/get-team-by-id/${team_id}`)
        .then((teamResponse) => {
          const teamInfo = teamResponse.data;
          setTeamData(teamInfo);
          
          // Get team members
          return apiClient.get(`teams/get-members-by-team/${team_id}`)
            .then((membersResponse) => {
                  ...
                  .then((profileResponse) => {
                    ...
                    try {
                      if (additionalProfile.social_links) {
                        socialLinks = JSON.parse(additionalProfile.social_links);
                      }
                    } catch (error) {
                      socialLinks = {};
                    }
                    ...
                    return {
                      ...
                      lastName: userData.lastName?.trim() || userProfile.lastName?.trim(),
                      email: userData.email || userProfile.email,
                      avatar: additionalProfile.profile_picture || additionalProfile.avatar,
                      ...
                      joinedAt: memberData.joinedAt || memberData.createdAt || userData.createdAt,
              ...
              return Promise.all(profilePromises).then((membersWithProfiles) => {
                setTeamMembers(membersWithProfiles);
                 // Filter team leader from members
                 if (teamInfo?.leaderId) {
                  const leader = membersWithProfiles.find(member => member.id === teamInfo.leaderId);
        ...
        <div className="relative mb-8 rounded-xl overflow-hidden shadow-lg">
          ...
          >
            <div className="text-center">
              ...
              <h3 className={`text-lg font-semibold ${teamData?.image ? 'text-white drop-shadow-lg' : 'text-primary/80'}`}>
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-4 pb-6 border-b border-border/40 pl-0 md:pl-32 mt-6">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              {teamData?.description || "Manage your team, projects and effective collaboration"}
            </p>
          </div>
        </header>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {teamProjects.map((project, index) => (
              <Card key={project.id || index} className="hover:shadow-lg transition-shadow duration-200">
                <CardHeader>
