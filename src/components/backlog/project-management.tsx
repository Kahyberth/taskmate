...
export default function ProjectManagement() {
  ...
  const handleStartSprint = (sprintId: string) => {
    setSprints(
      sprints.map((sprint) =>
        sprint.id === sprintId ? { ...sprint, isActive: true } : sprint
      )
    );
  ...
  const handleCompleteSprint = (sprintId: string) => {
    ...
    const completedTasks = currentSprint.tasks.filter(
      (task) => task.status === "done" || task.isCompleted
    );
    const incompleteTasks = currentSprint.tasks.filter(
      (task) => task.status !== "done" && !task.isCompleted
    );
  ...
  const openEditSprintModal = (sprint: Sprint) => {
    ...
    setEditSprintStartDate(undefined); // Aquí podrías establecer las fechas si las tuvieras en el objeto sprint
    setEditSprintEndDate(undefined);
    setEditSprintGoal(""); // Aquí podrías establecer la meta si la tuvieras en el objeto sprint
  ...
  const handleSaveSprintEdit = () => {
    ...
    const updatedSprint: Sprint = {
      ...editingSprint,
      name: editSprintName,
      // Aquí podrías actualizar más propiedades como fechas y meta si las tuvieras en el objeto sprint
    };

    setSprints(
      sprints.map((s) => (s.id === editingSprint.id ? updatedSprint : s))
    );
  ...
  const moveTaskToSprint = (taskId: string, targetSprintId: string) => {
    ...
    const updatedTask = taskToMove.isCompleted
      ? { ...taskToMove, isCompleted: false, status: "todo" }
      : taskToMove;
  ...
  const handleSaveTaskEdit = () => {
    ...
    const updatedTask: Task = {
      ...
      assignedTo:
        editTaskAssignedTo === "unassigned" ? undefined : editTaskAssignedTo,
    };
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <header className="border-b bg-white">
        ...
        <div className="flex items-center justify-between px-4 pb-2">
          <div className="flex items-center gap-2">
            <div className="flex items-center justify-center w-5 h-5 bg-red-500 text-white text-xs rounded">
              T
            </div>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={(e) => {
                  e.stopPropagation();
                  {
                    editingSprint && openEditSprintModal(editingSprint);
                  }
                }}
              >
      ...
      <div className="flex-1 overflow-auto p-4">
        ...
        {sprints.map((sprint) => (
          <div
            className="mb-6 border border-gray-200 rounded-md p-3 shadow-sm bg-white"
            key={sprint.id}
          >
            <div
            ...
            >
              ...
              <span className="ml-2 text-gray-500 text-sm">
                ...
                ({sprint.tasks.length} actividades)
              </span>
              <div className="ml-auto flex items-center gap-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem
                      onClick={() => {
                        // Aquí puedes implementar la edición del sprint
                        toast({
                          title: "Editar sprint",
                          description:
                            "Esta funcionalidad será implementada próximamente.",
                  ...
                  <div className="border rounded-md mb-1" key={task.id}>
                    <div className="flex items-center p-3 hover:bg-gray-50">
                      <Checkbox
                        ...
                        onCheckedChange={() =>
                          toggleTaskCompletion(sprint.id, task.id)
                        }
                      />
                      <label
                        htmlFor={task.id}
        ...
        <div className="border border-gray-200 rounded-md p-3 shadow-sm bg-white mb-6">
          ...
          >
            ...
            <span className="ml-2 text-gray-500 text-sm">
              ({backlogTasks.length} actividades)
            </span>
            <div className="ml-auto flex items-center gap-2">
              <span className="text-xs text-gray-500">0</span>
              <div className="w-6 h-4 bg-blue-100 rounded-sm flex items-center justify-center text-xs text-blue-800">
                ...
                <div className="border rounded-md mb-1" key={task.id}>
                  <div className="flex items-center p-3 hover:bg-gray-50">
                    <Checkbox
                      ...
                      checked={task.isCompleted}
                      onCheckedChange={() =>
                        toggleBacklogTaskCompletion(task.id)
                      }
                    />
                    <label
                      htmlFor={task.id}
                        ...
                        <DropdownMenuContent>
                          {sprints.map((sprint) => (
                            <DropdownMenuItem
                              key={sprint.id}
                              onClick={() =>
                                moveTaskToSprint(task.id, sprint.id)
                              }
                            >
                              {sprint.name}
                            </DropdownMenuItem>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setEditTaskStatus(value as "todo" | "in-progress" | "done")
                }
              >
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setEditTaskStoryPoints(Number.parseInt(value))
                }
              >
                <SelectTrigger className="col-span-3">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {editSprintStartDate ? (
                      format(editSprintStartDate, "PPP")
                    ) : (
                      <span>Seleccionar fecha</span>
                    )}
                  </Button>
